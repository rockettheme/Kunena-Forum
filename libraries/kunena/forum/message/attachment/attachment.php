		...
		if (!isset($this->_shorttype)) {
			...
			switch (strtolower ( $this->_shorttype )) {
				case 'image' :
					...
					$this->_thumblink = $this->_getAttachementLink ( $this->escape ( $this->folder ), $this->escape ( $this->filename ), $img, $this->escape ( $this->filename ), '' );
					...
					$this->_imagelink = $this->_getAttachementLink ( $this->escape ( $this->folder ), $this->escape ( $this->filename ), $img, $this->escape ( $this->filename ), '' );
...
*/
	...
	protected function _getAttachementLink($folder, $filename, $name, $title = '', $rel = 'nofollow') {
		...
		return '<a data-modal href="'.$link.'" title="'.$title.'" rel="'.$rel.'">'.$name.'</a>';
	}
}
